...
class BoardPanel extends TilePane {
	...
	BoardPanel(final int rows, final int columns, final int cellSize) {
		setPrefRows(rows);
	...
	void setImage(int row, int column, Image image, Element element) {
		cells[row][column].getChildren().add(new ImageView(image));

		if(element.isSpecial()) {
			if(element instanceof TimerCandy) {
