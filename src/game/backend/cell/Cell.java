...
public class Cell {
	...
	public boolean fallUpperContent() {
		...
		if (this.isEmpty() && !up.isEmpty() && up.isMovable()) {
			...
			Element e = up.getAndClearContent();

			while(up instanceof CandyGeneratorCell && e.isSpecial() && !grid.canEmitSpecial()) {
				e = up.getAndClearContent();
			}


			if(e.isSpecial() && up instanceof CandyGeneratorCell) {
				grid.addedSpecial(e);
			}
