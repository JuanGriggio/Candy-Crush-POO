...
public abstract class Grid {
	...
	protected abstract void setLevelCharacteristics();

	//////////////////////

	public List<Element> specials = new LinkedList<>();
	private int specialsLeftToEmit = 0;
	private int specialsLeft = 0;

	protected void setSpecialsAmount(int n) {
		if(n < 0)
			throw new IllegalStateException("Can't emit less than zero specials!");
		specialsLeft = n;
		specialsLeftToEmit = n;
	}
	...
	public int getSpecialsLeftToEmit () {
		return specialsLeftToEmit;
	}

	public void addedSpecial (Element e) {
		specials.add(e);
		specialsLeftToEmit--;
	}

	public void removedSpecial (Element e) {
		specials.remove(e);
		specialsLeft--;
	}

	public boolean canEmitSpecial () {
		return specialsLeftToEmit > 0;
	}
	...
	private void removeFigure(int i, int j, Figure f) {
		...
		if (f.hasReplacement()) {
			if(g[i][j].getContent().isSpecial()) {
				removedSpecial(g[i][j].getContent());
			}
